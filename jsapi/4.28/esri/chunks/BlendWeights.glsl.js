...
//>>built
define(["exports","../views/3d/webgl-engine/core/shaderLibrary/ScreenSpacePass.glsl","../views/3d/webgl-engine/core/shaderModules/interfaces","../views/3d/webgl-engine/core/shaderModules/ShaderBuilder","../views/3d/webgl-engine/core/shaderModules/Texture2DPassUniform"],function(f,h,a,k,e){function g(){const b=new k.ShaderBuilder;b.include(h.ScreenSpacePass);b.fragment.uniforms.add(new e.Texture2DPassUniform("edgesTexture",d=>d.inputTexture));b.fragment.uniforms.add(new e.Texture2DPassUniform("areaTexture",
d=>d.areaTexture));b.fragment.uniforms.add(new e.Texture2DPassUniform("searchTexture",d=>d.searchTexture));b.fragment.code.add(a.glsl`
      ...
      float searchXLeft( sampler2D edgesTex, sampler2D searchTex, vec2 texcoord, float end, vec2 resolution ) {
        vec2 e = vec2( 0.0, 1.0 );
        for ( int i = 0; i < ${a.glsl.int(c.maxSearchSteps)}; i ++ ) {
      ...
      vec2 getArea( sampler2D areaTex, vec2 dist, float e1, float e2, float offset ) {
        vec2 texcoord = float( ${a.glsl.int(c.maxDistanceAreaTex)} ) * round( 4.0 * vec2( e1, e2 ) ) + dist;
      ...
      void main() {
        ...
        vec4 maxOffset = vec4( offsets[0].xz, offsets[1].yw ) + vec4( -2.0, 2.0, -2.0, 2.0 ) * resolution.xxyy * float( ${a.glsl.int(c.maxSearchSteps)} );
    ...
    `);return b}const c={maxSearchSteps:8,maxDistanceAreaTex:16},l=Object.freeze(Object.defineProperty({__proto__:null,build:g},Symbol.toStringTag,{value:"Module"}));f.BlendWeights=l;f.build=g});