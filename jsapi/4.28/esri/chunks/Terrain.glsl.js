...
return normalize(n);
}`)};r.addProjViewLocalOrigin(e,a);c.include(I.Transform,a);const h=a.overlayMode!==k.OverlayMode.Disabled;var f=h&&a.invisible;switch(a.output){case g.ShaderOutput.Color:c.include(A.TerrainTexture,a);c.include(q.EvaluateSceneLighting,a);h&&c.include(k.OverlayTerrain,{...a,pbrMode:a.pbrMode===m.PBRMode.Terrain?m.PBRMode.TerrainWithWater:m.PBRMode.Water});(f=a.overlayMode===k.OverlayMode.EnabledWithWater)&&c.include(M.VertexTangent,a);n.add("vnormal","vec3");n.add("vpos","vec3");n.add("vup","vec3");
...
return (min(min(c.r, c.g), c.b) + max(max(c.r, c.g), c.b)) * 0.5;
      ...
      `);break;case g.ShaderOutput.Depth:f&&c.include(k.OverlayTerrain,a);c.include(y.OutputDepth,a);p.addLinearDepth(c);p.addNearFar(c);e.code.add(b.glsl`
              void main(void) {
